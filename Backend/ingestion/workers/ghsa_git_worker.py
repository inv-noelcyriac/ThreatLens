...
class GHSAGitTask(BaseIngestionTask):
    ...
    Supports delta updates and re-normalization triggers via database upserts.
    """
        ...
        try:
            ...
            for json_file in repo_path.glob("**/*.json"):
                ...
                if len(batch_buffer) >= BATCH_SIZE:
                    ...
                    logger.info(
                        f"[GHSA] Bulk-upserted batch of {len(batch_buffer)} items | Scanned: {total_scanned:,}"
                    )
            ...
            if batch_buffer:
                ...
                total_processed += len(batch_buffer)
                logger.info(f"[GHSA] Final batch bulk-upserted ({len(batch_buffer)} items).")
                batch_buffer.clear()
